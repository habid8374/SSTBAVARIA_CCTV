...
function DiasAlerta({ token }: { token: string }) {
  ...
  async function guardar(event: FormEvent) {
    ...
    setEnviando(true);
    try {
      const actualizado = await actualizarConfiguracionAlertas(token, { dias_alerta_vencimiento: Number(dias) });
  ...
  return (
    <div className="rounded-xl border border-corp-border bg-white p-5 shadow-sm">
      <h3 className="text-sm font-semibold text-corp-navy">Días de alerta de vencimiento</h3>
      <p className="mt-1 text-sm text-corp-muted">
        A cuántos días de vencer una planilla de seguridad social se considera &quot;por vencer&quot; en el
        banner de aviso de Contratistas.
      </p>
      {config && (
        <form onSubmit={guardar} className="mt-3 flex items-end gap-3">
          <label className="space-y-1.5">
            <span className="text-sm font-medium text-corp-navy">Días</span>
            <input
              type="number"
              min={1}
              required
